...
export const intersection = (
    intervals1: Tuple<number>[],
    intervals2: Tuple<number>[],
): Tuple<number>[] => {
    let index1 = 0,
        index2 = 0;
    const getIntersectionAndUpdateIndexes = (
        i1e: number,
        i2e: number,
        is: number,
    ): Tuple<number> => {
    ...
    if (intervals1.length === 0 || intervals2.length === 0) return res;
    while (index1 < intervals1.length && index2 < intervals2.length) {
        ...
        if (interval1Start >= interval2End) index2++;
        else if (interval2Start >= interval1End) index1++;
        else if (interval1Start > interval2Start)
            res.push(
                getIntersectionAndUpdateIndexes(
                    interval1End,
                    interval2End,
                    interval1Start,
                ),
