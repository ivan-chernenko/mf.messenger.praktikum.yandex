import {PlainObject} from '../object-helpers/types';
import {isObject} from "../object-helpers";

export const set = (objToSet: PlainObject | unknown, path: string, value: unknown): PlainObject | unknown => {
    if (path.length === 0 || typeof path !== "string")
        throw new Error('path must be string');
    if (!isObject(objToSet))
        return objToSet;
