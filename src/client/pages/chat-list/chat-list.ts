...
import {Chat, ChatProps} from "../../components/chat/index";
...
export class ChatListPage extends Page<ChatListPageProps> {
    ...
    deleteUser = () => {
        const login = this.getContent().querySelector('[data-name="delete-user-input"]');
        if (login && this.props.selectedChat)
            this.props.chatListController.deleteUser(this.props.selectedChat, (login as HTMLInputElement).value)
                .catch(this.deleteOrAddUserFailed)
    };

    addUser = () => {
        const login = this.getContent().querySelector('[data-name="add-user-input"]');
        if (login && this.props.selectedChat)
            this.props.chatListController.addUser(this.props.selectedChat, (login as HTMLInputElement).value)
                .catch(this.deleteOrAddUserFailed)
    };
    ...
    getChats = () => {
        this.props.chatListController.getChatList()
    ...
    createChat = () => {
        const createChatInput = this.getContent().querySelector('[data-name="new-chat-input"]');
        if (!createChatInput)
            return;
        this.props.chatListController.createChat({title: (createChatInput as HTMLInputElement).value})
            .then(this.getChats)
    ...
    componentDidRender() {
        const createChatButton = this.children.find(ch => ch.getName() === 'new-chat-button');
        const addUserButton = this.children.find(ch => ch.getName() === 'add-user-button');
        const deleteUserButton = this.children.find(ch => ch.getName() === 'delete-user-button');
        if (!createChatButton || !addUserButton || !deleteUserButton)
            return;
